...
const CreateSubCategoryModal = memo(({ open, onClose }: Props) => {
  const subCategoryActions = useSubCategoryActions();
  const { isCreating } = useSubCategoryState();
  ...
  async function onSubmit(data: ISubCategoryBody) {
    console.log("CreateSubCategoryModal: data", data);
    const { id, name } = data;
    ...
    if (subCategoryActions) {
      try {
        await subCategoryActions.createNewSubCategory(payload);
        handleClose(); 
      } catch (err) {
        console.error(err);     
      }
